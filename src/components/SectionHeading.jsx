...
export default function SectionHeading({ eyebrow, title, subtitle, align = 'center', light }) {
  ...
  return (
    ...
    >
      ...
      <h2 className={`heading-serif text-3xl sm:text-4xl md:text-5xl ${light ? 'text-black' : 'text-ink'} leading-tight`}>{title}</h2>
      <div className={align === 'center' ? 'gold-divider' : 'h-[1px] w-16 bg-gold-gradient bg-[length:200%_100%]'} />
      {subtitle && <p className={`text-sm sm:text-base ${light ? 'text-black/60' : 'text-ink/60'} leading-relaxed`}>{subtitle}</p>}
    </motion.div>
  )
}
