    ...
    <>
      ...
      <section className="pb-24 sm:pb-32 bg-pearl">
        <div className="container-lux">
          <div className="flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between mb-10 sticky top-24 z-30 bg-pearl/80 backdrop-blur-xl py-4 border-y border-gold/10">
              ...
              {categories.map((c) => (
                <button
                  ...
                  className={`px-4 py-2 text-[11px] tracking-widest2 uppercase border transition-all duration-300 ${
                    category === c ? 'bg-gold text-black border-gold' : 'border-gold/25 text-ink/60 hover:border-gold hover:text-gold'
                  }`}
                >
                  {c}
                </button>
            ...
            <div className="flex gap-3">
              <input
                ...
                className="bg-transparent border border-gold/25 px-4 py-2 text-sm text-ink placeholder:text-ink/30 focus:border-gold outline-none w-44 sm:w-56"
              />
              <select
                value={sort}
                onChange={(e) => setSort(e.target.value)}
                className="bg-pearl border border-gold/25 px-3 py-2 text-sm text-ink focus:border-gold outline-none"
              >
          ...
          {filtered.length === 0 ? (
            <p className="text-center text-ink/40 py-20">No products match your search. Try a different filter.</p>
          ) : (
