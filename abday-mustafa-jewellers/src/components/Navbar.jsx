  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled ? 'bg-black/85 backdrop-blur-xl border-b border-gold/15 py-3' : 'bg-gradient-to-b from-black/70 to-transparent py-6'
      }`}
    >
      <nav className="container-lux flex items-center justify-between">
        <NavLink to="/" className="flex items-center gap-3 group" data-cursor="link">
          <span className="w-10 h-10 border border-gold/60 rounded-full flex items-center justify-center text-gold heading-serif text-lg group-hover:bg-gold group-hover:text-black transition-all duration-500">
            AM
          </span>
          <span className="flex flex-col leading-none">
            <span className="heading-serif text-lg sm:text-xl tracking-wide text-ivory">Abday Mustafa</span>
        ...
        <a href="tel:+923057121111" className="hidden lg:inline-flex btn-outline !py-2.5 !px-6 !text-[11px]" data-cursor="link">
          Call Now
        </a>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="lg:hidden bg-black/98 backdrop-blur-xl border-t border-gold/10 overflow-hidden"
          >
            <ul className="flex flex-col py-6 px-8 gap-5">
              {links.map((l) => (
                <li key={l.to}>
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `block text-sm tracking-[0.2em] uppercase py-1 ${isActive ? 'text-gold' : 'text-ivory/80'}`
                    }
                  >
                    {l.label}
                  </NavLink>
              ...
              <a href="tel:+923057121111" className="btn-gold mt-2 !text-[11px]">Call Now</a>
            </ul>
          </motion.div>
        )}
      </AnimatePresence>
